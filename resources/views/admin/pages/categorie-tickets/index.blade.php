...
@section('content')
    <div class="br-mainpanel">
        <div class="pd-30">
            <h4 class="tx-gray-800 mg-b-5">Liste des Catégories de Tickets</h4>
        </div><!-- d-flex -->
                        ...
                        <div class="pd-x-20 pd-t-30 pd-b-15">
                            ...
                            <div class="table-wrapper">
                                <table id="datatable1" class="table display table-responsive nowrap">
                                    ...
                                    <tbody>
                                        @foreach ($categories_tickets as $categorie_ticket)
                                            <tr>
                                                <td class="wd-5p">{{ $categorie_ticket->id }}</td>
                                                <td class="wd-15p">{{ $categorie_ticket->name }}</td>
                                                <td class="wd-20p">{{ $categorie_ticket->first_name . ' ' . $categorie_ticket->last_name }}
                                                </td>
                                                <td class="wd-20p">{{ $categorie_ticket->email }}</td>
                                                <td class="wd-20p">{{ Carbon\Carbon::parse($categorie_ticket->add_date)->locale('fr_FR')->isoFormat('LLLL') }}
                                                </td>
                                                {{-- <td>{{ Carbon\Carbon::parse($categorie_ticket->edit_date)->locale('fr_FR')->isoFormat('LLLL') }}
                                                </td> --}}
                                                <th class="wd-20p" style="cursor:pointer">
                                                    <a href="{{ route('categories_tickets.edit', $categorie_ticket->id) }}"
                                                        class="btn btn-info btn-sm">Modifier</a>
                                                    <a href="javascript:;"
                                                        onclick="deleteFunction('{{ $categorie_ticket->id }}')"
                            ...
                            <!-- BASIC MODAL -->
                            <div class="modal fade" id="modal-categorie_ticket">
                                <div class="modal-dialog  modal-lg">
                                    <form id="FormCategorieTicket" action="{{ route('categories_tickets.store') }}"
                                        ...
                                        <div class="modal-content modal-dialog-vertical-center" style="width: 500px;">
                                            ...
                                            <div class="modal-body">
                                                <div class="col-lg">
                                                    <label class="form-control-label">Catégories ticket: <span
...
@section('scripts')
    <script>
        ...
        function addRole() {
            $('#modal-categorie_ticket').modal('show');
            $('#FormCategorieTicket').trigger('reset');
        }


        function deleteFunction(CategorieTicket_id) {
            // alert(categorie_id);
            Swal.fire({
                title: "Êtes-vous sûr?",
                ...
                if (result.value) {
                    $.ajax({
                        method: "POST",
                        url: "/delete-CategoriesTickets/",
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                        data: {
                            CategorieTicket_id: CategorieTicket_id
                        },
                        dataType: 'json',
